...
for i in $all_tests; do
    ...
    then
        ...
        printf "\n";
        # sed -i '1i***EXPECT***' $EXPECT    # add in headers for expect actual
        # sed -i '1i***ACTUAL***' $ACTUAL
        $DIFF <(sed '1i***EXPECT***' $EXPECT) <(sed '1i***ACTUAL***' $ACTUAL) > $DIFFOUT   # regen the diff with the headers
        cat $DIFFOUT
        if [ "$generate" == "1" ]; then
            printf "\nFULL ACTUAL\n"
            # printf "\n"
